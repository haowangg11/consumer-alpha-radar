...
def _eastmoney_kline(code: str, category: int = 4, offset: int = 60) -> list[dict]:
    """东财 K 线兜底：category 4=日 5=周 6=月 11=60分钟。"""
...
def _tencent_kline(code: str, category: int = 4, offset: int = 60) -> list[dict]:
    """腾讯 K 线兜底。公开接口稳定，字段为 date/open/close/high/low/volume。"""
    import requests

    prefix = get_prefix(code)
    period = {4: "day", 5: "week", 6: "month", 11: "m60"}.get(category, "day")
    adjust_prefix = "qfq" if period in ("day", "week", "month") else ""
    key = f"{adjust_prefix}{period}" if adjust_prefix else period
    params = {"param": f"{prefix}{code},{period},,,{offset},qfq"}
    try:
        data = requests.get(
            ...
            timeout=15,
        ).json().get("data", {}).get(f"{prefix}{code}", {})
    except Exception:
        return []
    raw_rows = data.get(key) or data.get(period) or []
    rows = []
    for row in raw_rows:
...
def kline(code: str, category: int = 4, offset: int = 60) -> list[dict]:
    """K线：category 4=日 5=周 6=月 11=60分钟。优先腾讯快源，失败再退回东财/mootdx。"""
    rows = _tencent_kline(code, category=category, offset=offset)
    if rows:
        return rows
    rows = _eastmoney_kline(code, category=category, offset=offset)
    if rows:
        return rows
